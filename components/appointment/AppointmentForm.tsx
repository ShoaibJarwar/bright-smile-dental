...
import { notifyAll, buildEmailMessage, buildWhatsAppMessage } from "@/lib/notifications";
...
export default function AppointmentForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const rows = [
      ...
      { label: "Doctor",      value: form.doctor     || "No preference"              },
      { label: "Date",        value: form.date       || "To be confirmed"            },
      { label: "Time",        value: form.time       || "Any available"              },
      { label: "First Visit", value: form.firstVisit === "yes" ? "Yes" : "Returning" },
    ...
    try {
      await notifyAll({
        ...
        emailMessage: buildEmailMessage("New Appointment Request", rows),
        waMessage:    buildWhatsAppMessage("New Appointment Request", rows),
      });
      setSubmitted(true);
    } catch (err) {
      console.error(err);
      setError(
        "Something went wrong. Please call us directly at +92 303 7224163."
      );
    } finally {
      setLoading(false);
    }
  ...
  if (submitted) {
    return (
      ...
      >
        ...
        <h3 className="font-display font-bold text-3xl" style={{ color: "var(--text)" }}>
          Appointment Requested!
        </h3>
        <p className="text-base max-w-md leading-relaxed" style={{ color: "var(--text-muted)" }}>
          Thank you, <strong>{form.name.split(" ")[0]}</strong>. Your request
          has been sent. We will confirm within 2 hours.
        </p>
        <div
        ...
        <button
          ...
          className="px-6 py-3 rounded-xl text-sm font-semibold border-2
                     border-primary-200 text-primary-500 hover:bg-primary-500/10
          ...
          <div key={s} className="flex items-center gap-3">
            ...
            >
              ...
              <span
                ...
                style={{ color: step >= s ? "var(--text)" : "var(--text-muted)" }}
              >
            ...
            {s < 2 && (
              <div
                ...
                style={{ background: step > s ? "#0EA5E9" : "var(--border)" }}
              />
            )}
          </div>
        ))}
        ...
        >
          <div className="grid sm:grid-cols-2 gap-4">
            <div>
              ...
              <input type="text" value={form.name} onChange={set("name")}
                placeholder="Bilal Ahmed" required className={inputClass} style={inputStyle} />
            </div>
            <div>
              <label className="block text-sm font-medium mb-2" style={{ color: "var(--text)" }}>
              ...
              <input type="tel" value={form.phone} onChange={set("phone")}
                placeholder="+92 300 0000000" required className={inputClass} style={inputStyle} />
            </div>
          </div>
          ...
          <div>
            ...
            <input type="email" value={form.email} onChange={set("email")}
              placeholder="you@email.com" required className={inputClass} style={inputStyle} />
          </div>
            ...
            <div className="flex gap-4">
              {["yes", "no"].map((val) => (
                <label key={val} className="flex items-center gap-2.5 cursor-pointer">
                  <input type="radio" name="firstVisit" value={val}
                    checked={form.firstVisit === val} onChange={set("firstVisit")}
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            <div>
              ...
              <input type="date" value={form.date} onChange={set("date")}
                min={today} required className={inputClass} style={inputStyle} />
            </div>
            <div>
              <label className="block text-sm font-medium mb-2" style={{ color: "var(--text)" }}>
                Preferred Time
          ...
          <div className="flex gap-3">
            <button type="button" onClick={() => setStep(1)}
              className="px-6 py-4 rounded-2xl text-sm font-semibold border-2
                         transition-colors hover:bg-primary-500/10 cursor-pointer"
            ...
            >
              ...
              {loading ? "Sending…" : "Confirm Appointment Request"}
            </button>
          </div>
        </motion.div>
      )}
