  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await notifyAll({
        ...
        emailMessage: buildEmailMessage("New Contact Message", rows),
        waMessage:    buildWhatsAppMessage("New Contact Message", rows),
      });
      setSubmitted(true);
    } catch (err) {
      console.error(err);
      setError(
        "Something went wrong. Please try again or call +92 303 7224163."
      );
    } finally {
      setLoading(false);
    }
    ...
    return (
      ...
      >
        ...
        <button
          ...
          className="mt-4 px-6 py-3 rounded-xl text-sm font-semibold text-primary-500
                     border-2 border-primary-200 hover:bg-primary-500/10
    ...
    >
      ...
      <motion.div variants={slideUp} className="grid sm:grid-cols-2 gap-4">
        <div>
          ...
          <input type="text" value={form.name} onChange={set("name")}
            placeholder="Ayesha Malik" required className={inputClass} style={inputStyle} />
        </div>
        <div>
          <label className="block text-sm font-medium mb-2" style={{ color: "var(--text)" }}>
      ...
      <motion.div variants={slideUp} className="grid sm:grid-cols-2 gap-4">
        <div>
          ...
          <input type="tel" value={form.phone} onChange={set("phone")}
            placeholder="+92 300 0000000" className={inputClass} style={inputStyle} />
        </div>
        <div>
          <label className="block text-sm font-medium mb-2" style={{ color: "var(--text)" }}>
      ...
      <motion.div variants={slideUp}>
        ...
        <textarea value={form.message} onChange={set("message")}
          ...
          required rows={5} className={inputClass + " resize-none"} style={inputStyle} />
      </motion.div>
